    ...
    <PageCard title="System Users" subtitle="Manage access and roles"
      ...
      <table className="w-full text-sm">
        <thead><tr className="text-left text-muted-foreground border-b border-border">
          ...
          <th className="py-2 pr-3">Role</th><th className="py-2 pr-3">Status</th><th className="py-2 w-10"></th>
        </tr></thead>
        <tbody>
          ...
          {(data ?? []).map((u: any) => (
            <tr key={u.id} className="border-b border-border/50 hover:bg-muted/30">
              ...
              <td className="py-2.5 pr-3"><Badge className={ROLE_COLORS[u.role] ?? ""}>{u.role.replace("_", " ")}</Badge></td>
              <td className="py-2.5 pr-3"><Badge className={u.is_active ? "bg-success/15 text-success border-success/30" : "bg-muted text-muted-foreground border-border"}>{u.is_active ? "Active" : "Inactive"}</Badge></td>
              <td className="py-2.5"><button onClick={() => { setEditing(u); setOpen(true); }} className="p-1.5 rounded hover:bg-muted"><Pencil className="size-4" /></button></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function UserForm({ initial, onClose, onSaved }: { initial: any; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({
    ...
    email: initial?.email ?? "", role: initial?.role ?? "teacher", is_active: initial?.is_active ?? true,
  });
  ...
  async function save() {
    ...
    try {
      ...
      if (initial) {
        ...
        if (!password) { toast.error("Password required for new user"); return; }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-card border border-border rounded-2xl w-full max-w-md p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label={initial ? "New password (leave blank to keep)" : "Password"} className="sm:col-span-2">
            <Input type="password" value={password} onChange={(v) => setPassword(v)} placeholder={initial ? "Leave blank to keep current" : "Set password"} />
          </Field>
        </div>
  ...
  return (
    <PageCard title="Institutions" subtitle="Schools and organisations billed as groups"
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading && <p className="text-muted-foreground col-span-3 py-6 text-center">Loading…</p>}
        {(data ?? []).map((inst: any) => (
          <div key={inst.id} className="rounded-xl border border-border bg-background p-4 space-y-2">
            ...
            <Badge className={inst.is_active ? "bg-success/15 text-success border-success/30" : "bg-muted text-muted-foreground border-border"}>
              {inst.is_active ? "Active" : "Inactive"}
            </Badge>
          </div>
        ))}
        {!isLoading && !data?.length && <p className="text-muted-foreground col-span-3 py-8 text-center">No institutions yet — add your first school or organisation</p>}
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-lg p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label="Notes" className="sm:col-span-2">
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} className="w-full bg-background border border-input rounded-md px-3 py-2 text-sm" />
          </Field>
        </div>
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-4 py-2 text-sm rounded-md hover:bg-muted">Cancel</button>
...
function TermsTab() {
  ...
  return (
    <PageCard title="Academic Terms" subtitle="Manage term dates and set the current term">
      <div className="space-y-2">
        {isLoading && <p className="text-muted-foreground py-6 text-center">Loading…</p>}
        {(data ?? []).map((t: any) => (
